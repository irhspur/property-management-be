...
export const getUser = async (userId: number): Promise<Record<string, any>> => {
  const user = await userModel.findSummaryById(userId);
  if (!user) throw err('No user found.', 404);
  return user;
};

export const getUserProfile = async (userId: number): Promise<Record<string, any>> => {
  const profile = await userModel.findProfileById(userId);
  if (!profile) throw err('No user found.', 404);
  return profile;
};

export const getAddress = async (userId: number): Promise<Record<string, any>> => {
  const address = await addressModel.findWithJoinsByUserId(userId);
  if (!address) throw err('No address found.', 404);
  return address;
};

export const updateDetails = async (userId: number, body: Record<string, any>): Promise<UserDetails> => {
...
export const updateAddress = async (userId: number, body: Record<string, any>): Promise<Address> => {
