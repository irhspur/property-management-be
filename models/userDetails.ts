...
export const upsert = async (userId: string, d: UserDetailsFields, client: DbClient = pool): Promise<UserDetails> => {
  const { rows } = await client.query<UserDetails>(
    ...
    ON CONFLICT (user_id) DO UPDATE SET
      first_name = INITCAP($2), middle_name = INITCAP($3), last_name = INITCAP($4),
      gender_id = $5, dob = $6, country_id = $7, birth_province_id = $8,
      birth_district_id = $9, father_full_name = INITCAP($10), nin_number = $11,
      mobile_number = $12, citizenship_number = $13,
      citizenship_issue_district_id = $14, citizenship_issue_date = $15,
      bank_account_number = $16, bank_name = INITCAP($17), updated_at = NOW()
    RETURNING *`,
    [
      userId,
      d.first_name, d.middle_name, d.last_name, d.gender_id, d.dob,
