...
export const createGender = async (req, res) => {
    try {
        const { name } = req.body;
        const gender = await pool.query("SELECT * FROM gender WHERE lower(name) = lower($1)", [name]);
        if (gender.rows.length > 0) {
            return res.status(400).json({status: "NAK", message: "Gender already exists"});
        }
        const newGender = await pool.query("INSERT INTO gender (name) VALUES (INITCAP($1)) RETURNING *", [name]);
...
export const updateGender = async (req, res) => {
    try {
        ...
        if (gender.rows.length === 0) {
            return res.status(404).json({status: "NAK", message: "Gender not found"});
        }
        const updatedGender = await pool.query("UPDATE gender SET name = (INITCAP($1)) RETURNING *", [name]);
