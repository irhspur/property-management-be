...
export const getUserTypeById = async (req, res) => {
    try {
        const { id } = req.params;
        const userType = await pool.query("SELECT * FROM user_type WHERE id = $1", [id]);
        if (userType.rows.length === 0) {
            return res.status(404).json({status: "NAK", message: "User type not found"});
        }
...
export const createUserType = async (req, res) => {
    try {
        const { name } = req.body;
        const userType = await pool.query("SELECT * FROM user_type WHERE lower(name) = lower($1)", [name]);
        if (userType.rows.length > 0) {
            return res.status(400).json({status: "NAK", message: "User type already exists"});
        }
        const newUserType = await pool.query("INSERT INTO user_type (name) VALUES (INITCAP($1)) RETURNING *", [name]);
...
export const updateUserType = async (req, res) => {
    try {
        ...
        const updatedUserType = await pool.query("UPDATE user_type SET name = INITCAP($1) WHERE id = $2 RETURNING *", [name, id]);
...
export const getUserTypeByName = async (req, res) => {
    try {
